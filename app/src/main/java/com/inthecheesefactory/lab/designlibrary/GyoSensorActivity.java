...
public class GyoSensorActivity extends AppCompatActivity implements SensorEventListener {
    Toolbar toolbar;
    ...
    private GravityPathView gravityPathView;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 1: {
                    //Log.v("handle", "here");
                    gravityPathView.setData(lux);
                    gravityPathView.invalidate();
                }
            }
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mSensor != null) {
            gravityPathView = new GravityPathView(this);
            RelativeLayout relativeLayout = (RelativeLayout) findViewById(R.id.view_layout);
            relativeLayout.addView(gravityPathView);

            thread = new Thread(new GyoSensorActivity.GameThread());
            thread.start();
        } else {
    ...
    private void initInstances() {
        ...
        drawerToggle = new ActionBarDrawerToggle(GyoSensorActivity.this, drawerLayout, R.string.hello_world, R.string.hello_world);
        drawerLayout.setDrawerListener(drawerToggle);

        getSupportActionBar().setHomeButtonEnabled(true);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle(R.string.txt_gyo);

        final Intent intent = new Intent(this, MainActivity.class);
        final Intent lightIntent = new Intent(this, LightSensorActivity.class);
        final Intent acceIntent = new Intent(this, AcceSensorActivity.class);
    ...
    class GameThread implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                gravityPathView.postInvalidate();

                //Send message
                GyoSensorActivity.this.mHandler.sendMessage(message);
                try {
                    Thread.sleep(100);
    ...
    class GravityPathView extends View {
        ...
        public GravityPathView(Context context) {
            this(context, null);
        }

        public GravityPathView(Context context, AttributeSet attrs) {
            this(context, attrs, 0);
        }

        public GravityPathView(Context context, AttributeSet attrs, int defStyleAttr) {
